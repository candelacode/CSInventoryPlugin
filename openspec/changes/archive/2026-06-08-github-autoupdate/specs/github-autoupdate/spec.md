...
- **THEN** `RepositoryName` returns `"jccan/BotManager.Plugin"`
...
- **THEN** it returns `typeof(BotManagementPlugin).Assembly.GetName().Version`
...
- **THEN** the release asset `BotManager.Plugin.zip` is attached
- **AND** the zip contains `BotManager.Plugin.dll` in its root directory
...
- **THEN** `BotManager.Plugin.dll` reports `Version` as `1.0.1.0`
