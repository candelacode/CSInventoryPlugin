...
When CS items are detected in a bot's inventory after a trade, the system SHALL send a trade offer containing those CS items to the configured master account.
...
- **AND** sends the trade offer via `Bot.SendTradeOffer()`
...
- **WHEN** `Bot.SendTradeOffer()` fails for any reason
